...
% \Vpacks{Biobase, annotate, geneplotter}
...
To make use of these tools you will need the packages: {\em biobase},
{\em annotate}, and {\em geneplotter}. These must be installed in your
version of R and when you start R you must load them with the
\verb+library+ command.
...
The functions \verb+cPlot+ and \verb+cColor+ allow the user to
...
The original setup is done using \verb+cPlot+. The subsequent coloring
is done using \verb+cColor+.


We will use the example data in \verb+eset+ to show how this function
might be used.
...
data(hgu95AProbLocs)
cPlot(hgu95AProbLocs)
cColor(geneNames(eset), cols[s1class],
        hgu95AProbLocs)
cPlot(hgu95AProbLocs)
cColor(geneNames(eset), cols[s2class],
        hgu95AProbLocs)
...
<<fig=TRUE, echo=FALSE>>=
 ...
 alongChrom(eset, "1", hgu95AProbLocs, plotFormat="cumulative", 
	col=mycols)
@
\end{center}
