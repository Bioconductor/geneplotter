...
In order to use the various \texttt{geneplotter} functions you will
need to assemble an object of class \texttt{chromLocation}.
...
\texttt{hu6800} data package to construct our object.
...
<<loaddata>>=
 ...
 chrs <- multiget(affy, env=hu6800CHR)
...
@
So somehow ten of the genes are mapped to two different chromosomes.
...
<<strandloc>>=
 ...
 strand <- multiget(affy, env=hu6800CHRLOC)

 gstr <- sapply(strand, function(x) as.numeric(x[1]))
 names(gstr) <- names(strand)

 orient <- multiget(affy, env=hu6800CHRORI)
 gorient <- sapply(orient, function(x) x[1])
 names(gorient) <- names(orient)

 gx<-ifelse(gorient=="-", -gstr, gstr)

 splits <- split(gx, chrs2)
 length(splits)
 names(splits)
...
\texttt{chromLocation} object.

<<chrloc>>=

 data(hgCLengths)

 newChrClass <- buildChromClass("Human", "Hu6800 Bioconductor",
     splits, hgCLengths)


@

And finally we can test it by calling \texttt{cPlot}.
